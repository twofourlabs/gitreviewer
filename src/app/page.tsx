...
const GITHUB_REPO = 'yagyesh-bobde/gitreview';

/** Compact star count, e.g. 1200 -> "1.2k", 12000 -> "12k". */
...
export default async function HomePage() {
  ...
  // Only surface the live star count once it climbs past double digits (>99);
  // below that, the static "Star on GitHub" call-to-action reads better.
  const stars = await getStarCount();
  const showStarCount = stars !== null && stars > 99;
  const starLabel = showStarCount ? formatStars(stars) : 'Star on GitHub';
        ...
        <nav>
          ...
          <div className="nav-cta">
            <a
              ...
              className="btn-nav btn-github"
              aria-label="Star GitReview on GitHub"
            >
              {/* GitHub mark */}
              ...
              <span className={`btn-github-label${showStarCount ? ' btn-github-count' : ''}`}>{starLabel}</span>
              {/* Star icon */}
